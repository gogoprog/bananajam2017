...
Application.init = function() {
	gengine.setWindowSize(Module.IntVector2(512,512));
...
Application.start = function(_engine) {
	...
	var e = Application.createBackground();
	Application.engine.addEntity(e);
	e = Application.createTree();
	Application.engine.addEntity(e);
	var this2 = Module.Vector3(0,-255.600000000000023,0);
	e.set_position(this2);
	Application.basket = Application.createBasket();
	Application.createSpawner(-100,200);
	Application.createSpawner(100,200);
	Application.createSpawner(80,150);
	Application.createSpawner(-120,120);
	Application.createSpawner(-80,100);
	Application.createSpawner(50,100);
	Application.createSpawner(20,230);
	Application.createSpawner(-20,230);
	Application.engine.addSystem(new ExitSystem(),0);
	Application.engine.addSystem(new systems_AudioSystem(),0);
	Application.engine.addSystem(new systems_BasketSystem(),0);
	Application.engine.addSystem(new systems_ColorSystem(),0);
	systems_AudioSystem.instance.playGameMusic();
};
Application.createSpawner = function(x,y) {
	var e = new gengine_Entity();
	e.add(new components_Spawner());
	var this1 = Module.Vector3(x,y,0);
	e.set_position(this1);
	Application.engine.addEntity(e);
	return e;
};
Application.createTree = function() {
	var e = new gengine_Entity();
	if(e.name != "tree") {
		var previous = e.name;
		e.name = "tree";
	...
	var ss = e.get(gengine_components_StaticSprite2D);
	var sprite = gengine.getResourceCache().getSprite2D("banana-tree.png",true);
	ss.object.setSprite(sprite);
	ss.object.setLayer(1);
	ss.object.setUseHotSpot(true);
	var this1 = Module.Vector2(0.5,0);
	ss.object.setHotSpot(this1);
	var this2 = Module.Vector3(2.2,2.2,1);
	var scale = this2;
	e.node.setScale(scale);
	return e;
};
Application.createBasket = function() {
	var e = new gengine_Entity();
	e.add(new gengine_components_StaticSprite2D());
	e.add(new components_Basket());
	var tmp = e.get(gengine_components_StaticSprite2D);
	var tmp1 = gengine.getResourceCache().getSprite2D("crate.png",true);
	tmp.setSprite(tmp1);
	e.get(gengine_components_StaticSprite2D).setLayer(1);
	var this1 = Module.Vector3(1,1,1);
	var scale = this1;
	e.node.setScale(scale);
	if(e.name != "basket") {
		var previous = e.name;
	...
	var this2 = Module.Vector3(0,-200,0);
	e.set_position(this2);
	return e;
};
Application.createBackground = function() {
	var e = new gengine_Entity();
	e.add(new gengine_components_StaticSprite2D());
	var tmp = e.get(gengine_components_StaticSprite2D);
	var tmp1 = gengine.getResourceCache().getSprite2D("background.png",true);
	tmp.setSprite(tmp1);
	e.get(gengine_components_StaticSprite2D).setLayer(0);
	return e;
};
Application.createCamera = function() {
	var e = new gengine_Entity();
	if(e.name != "camera") {
		var previous = e.name;
		e.name = "camera";
	...
	e.add(new gengine_components_Camera());
	var tmp = e.get(gengine_components_Camera);
	var this1 = Module.Vector2(512,512);
	tmp.setOrthoSize(this1);
	e.get(gengine_components_Camera).setOrthographic(true);
	Application.engine.addEntity(e);
	var viewport = new Module.Viewport(gengine.getContext());
	viewport.setScene(gengine.getScene());
	viewport.setCamera(e.get(gengine_components_Camera).object);
...
Application.onGuiLoaded = function() {
	...
	$(".menu").click(function(e) {
		Application.engine.addEntity(Application.basket);
		Application.engine.addSystem(new systems_SpawnSystem(),0);
		Application.engine.addSystem(new systems_FallSystem(),0);
		Application.engine.addSystem(new systems_GrowSystem(),0);
		Application.engine.addSystem(new systems_ShakeSystem(),0);
		Application.pages.showPage(".hud");
	});
};
Application.gameOver = function() {
	Application.engine.updateComplete.addOnce(function() {
		Application.engine.removeEntity(Application.basket);
		Application.engine.removeSystem(Application.engine.getSystem(systems_SpawnSystem));
		Application.engine.removeSystem(Application.engine.getSystem(systems_FallSystem));
		Application.engine.removeSystem(Application.engine.getSystem(systems_GrowSystem));
		Application.engine.removeSystem(Application.engine.getSystem(systems_ShakeSystem));
		Application.pages.showPage(".menu");
	});
};
...
var components_Banana = function() {
	this.good = true;
};
components_Banana.__name__ = ["components","Banana"];
components_Banana.prototype = {
	__class__: components_Banana
};
var components_Basket = function() {
};
components_Basket.__name__ = ["components","Basket"];
components_Basket.prototype = {
	__class__: components_Basket
};
var components_Colorizer = function(duration,color) {
	this.time = 0;
	this.duration = duration;
	this.color = color;
};
components_Colorizer.__name__ = ["components","Colorizer"];
components_Colorizer.prototype = {
	__class__: components_Colorizer
};
var components_Fall = function() {
	this.startY = 0;
	this.time = 0;
};
components_Fall.__name__ = ["components","Fall"];
components_Fall.prototype = {
	__class__: components_Fall
};
var components_Grower = function() {
	this.time = 0;
};
components_Grower.__name__ = ["components","Grower"];
components_Grower.prototype = {
	__class__: components_Grower
};
var components_Shaker = function(duration,fall,intensity,interval) {
	this.fall = true;
	this.time = 0;
	this.timeLeft = 0;
	this.duration = duration;
...
components_Shaker.__name__ = ["components","Shaker"];
components_Shaker.prototype = {
	__class__: components_Shaker
};
var components_Spawner = function() {
	this.timeLeft = 0;
	this.time = 0;
};
components_Spawner.__name__ = ["components","Spawner"];
components_Spawner.prototype = {
	__class__: components_Spawner
};
...
var systems_AudioSystem = function() {
	...
	this.add("fail");
	this.add("hurt");
	systems_AudioSystem.instance = this;
};
...
systems_AudioSystem.prototype = $extend(ash_core_System.prototype,{
	...
	,addToEngine: function(_engine) {
		...
		this.musicSource = new gengine_components_SoundSource();
		this.musicSource.object.setSoundType("Music");
		this.musicSource.object.setGain(0.4);
		e1.add(this.musicSource);
	}
	,playSound: function(sound,position) {
		var ss = this.soundSources[this.nextSoundSourceIndex++];
		var _this = this.sounds;
		var sound1 = __map_reserved[sound] != null ? _this.getReserved(sound) : _this.h[sound];
		ss.object.play1(sound1);
		ss.object.setGain(0.7);
		this.nextSoundSourceIndex %= this.soundSources.length;
	}
	,playGameMusic: function() {
		var s = gengine.getResourceCache().getSound("ramones.ogg",true);
		s.setLooped(true);
		this.musicSource.object.play1(s);
	}
...
var systems_BasketNode = function() { };
systems_BasketNode.__name__ = ["systems","BasketNode"];
systems_BasketNode._getComponents = function() {
	if(systems_BasketNode._components == null) {
		systems_BasketNode._components = new ash_ClassMap();
		var _this = systems_BasketNode._components;
		var k = components_Basket;
		var name = Type.getClassName(k);
		var _this1 = _this.keyMap;
		if(__map_reserved[name] != null) {
			_this1.setReserved(name,k);
		...
		var _this2 = _this.valueMap;
		if(__map_reserved[name] != null) {
			_this2.setReserved(name,"basket");
		} else {
			_this2.h[name] = "basket";
		}
	}
	return systems_BasketNode._components;
};
systems_BasketNode.__super__ = ash_core_Node;
systems_BasketNode.prototype = $extend(ash_core_Node.prototype,{
	__class__: systems_BasketNode
});
var systems_BasketSystem = function() {
	ash_tools_ListIteratingSystem.call(this,systems_BasketNode,$bind(this,this.updateNode),$bind(this,this.onNodeAdded),$bind(this,this.onNodeRemoved));
};
systems_BasketSystem.__name__ = ["systems","BasketSystem"];
systems_BasketSystem.__super__ = ash_tools_ListIteratingSystem;
systems_BasketSystem.prototype = $extend(ash_tools_ListIteratingSystem.prototype,{
	addToEngine: function(engine) {
		ash_tools_ListIteratingSystem.prototype.addToEngine.call(this,engine);
		this.engine = engine;
		this.camera = engine.entityNames.get("camera").get(gengine_components_Camera);
	}
	,updateNode: function(node,dt) {
		var input = gengine.getInput();
		var mousePosition = input.getMousePosition();
		var this1 = Module.Vector2(mousePosition.x / 512,mousePosition.y / 512);
		...
		var p = node.entity.get_position();
		p.x = mouseWorldPosition.x;
		node.entity.set_position(p);
	}
	,onNodeAdded: function(node) {
		node.basket.score = 0;
		node.basket.life = 5;
		$(".score").text(Std.string(node.basket.score));
		$(".life").text(Std.string(node.basket.life));
	}
	,onNodeRemoved: function(node) {
	}
	,__class__: systems_BasketSystem
});
var systems_ColorizerNode = function() { };
systems_ColorizerNode.__name__ = ["systems","ColorizerNode"];
systems_ColorizerNode._getComponents = function() {
	if(systems_ColorizerNode._components == null) {
		systems_ColorizerNode._components = new ash_ClassMap();
		var _this = systems_ColorizerNode._components;
		var k = components_Colorizer;
		var name = Type.getClassName(k);
		var _this1 = _this.keyMap;
		if(__map_reserved[name] != null) {
			_this1.setReserved(name,k);
		...
		var _this2 = _this.valueMap;
		if(__map_reserved[name] != null) {
			_this2.setReserved(name,"colorizer");
		} else {
			_this2.h[name] = "colorizer";
		}
		var _this3 = systems_ColorizerNode._components;
	...
	return systems_ColorizerNode._components;
};
systems_ColorizerNode.__super__ = ash_core_Node;
systems_ColorizerNode.prototype = $extend(ash_core_Node.prototype,{
	__class__: systems_ColorizerNode
});
var systems_ColorSystem = function() {
	ash_tools_ListIteratingSystem.call(this,systems_ColorizerNode,$bind(this,this.updateNode),$bind(this,this.onNodeAdded),$bind(this,this.onNodeRemoved));
};
systems_ColorSystem.__name__ = ["systems","ColorSystem"];
systems_ColorSystem.__super__ = ash_tools_ListIteratingSystem;
systems_ColorSystem.prototype = $extend(ash_tools_ListIteratingSystem.prototype,{
	addToEngine: function(engine) {
		ash_tools_ListIteratingSystem.prototype.addToEngine.call(this,engine);
		this.engine = engine;
	}
	,updateNode: function(node,dt) {
		...
		if(f < 0.5) {
			var f1 = f * 2;
			var a = node.colorizer.initialColor;
			var a1 = node.colorizer.color;
			var b = node.colorizer.initialColor;
			var this1 = Module.Color(a1.r - b.r,a1.g - b.g,a1.b - b.b,a1.a - b.a);
			var a2 = this1;
			var this2 = Module.Color(a2.r * f1,a2.g * f1,a2.b * f1,a2.a * f1);
			var b1 = this2;
			var this3 = Module.Color(a.r + b1.r,a.g + b1.g,a.b + b1.b,a.a + b1.a);
			color = this3;
		} else if(f < 1) {
			var f2 = (f - 0.5) * 2;
			var a3 = node.colorizer.color;
			var a4 = node.colorizer.initialColor;
			var b2 = node.colorizer.color;
			var this4 = Module.Color(a4.r - b2.r,a4.g - b2.g,a4.b - b2.b,a4.a - b2.a);
			var a5 = this4;
			var this5 = Module.Color(a5.r * f2,a5.g * f2,a5.b * f2,a5.a * f2);
			var b3 = this5;
			var this6 = Module.Color(a3.r + b3.r,a3.g + b3.g,a3.b + b3.b,a3.a + b3.a);
			color = this6;
		} else {
			color = node.colorizer.initialColor;
			node.entity.remove(components_Colorizer);
		}
		node.sprite.object.setColor(color);
	}
	,onNodeAdded: function(node) {
		node.colorizer.time = 0;
		var tmp = node.sprite.object.getColor();
		node.colorizer.initialColor = tmp;
	}
	,onNodeRemoved: function(node) {
	}
	,__class__: systems_ColorSystem
});
var systems_FallNode = function() { };
systems_FallNode.__name__ = ["systems","FallNode"];
systems_FallNode._getComponents = function() {
	if(systems_FallNode._components == null) {
		systems_FallNode._components = new ash_ClassMap();
		var _this = systems_FallNode._components;
		var k = gengine_components_StaticSprite2D;
		var name = Type.getClassName(k);
		var _this1 = _this.keyMap;
		if(__map_reserved[name] != null) {
			_this1.setReserved(name,k);
		...
		var _this3 = systems_FallNode._components;
		var k1 = components_Fall;
		var name1 = Type.getClassName(k1);
		var _this4 = _this3.keyMap;
		if(__map_reserved[name1] != null) {
			_this4.setReserved(name1,k1);
		...
		var _this5 = _this3.valueMap;
		if(__map_reserved[name1] != null) {
			_this5.setReserved(name1,"fall");
		} else {
			_this5.h[name1] = "fall";
		}
		var _this6 = systems_FallNode._components;
		var k2 = components_Banana;
		...
		if(__map_reserved[name2] != null) {
			_this8.setReserved(name2,"banana");
		} else {
			_this8.h[name2] = "banana";
		}
	}
	return systems_FallNode._components;
};
systems_FallNode.__super__ = ash_core_Node;
systems_FallNode.prototype = $extend(ash_core_Node.prototype,{
	__class__: systems_FallNode
});
var systems_FallSystem = function() {
	ash_tools_ListIteratingSystem.call(this,systems_FallNode,$bind(this,this.updateNode),$bind(this,this.onNodeAdded),$bind(this,this.onNodeRemoved));
};
systems_FallSystem.__name__ = ["systems","FallSystem"];
systems_FallSystem.__super__ = ash_tools_ListIteratingSystem;
systems_FallSystem.prototype = $extend(ash_tools_ListIteratingSystem.prototype,{
	addToEngine: function(engine) {
		ash_tools_ListIteratingSystem.prototype.addToEngine.call(this,engine);
		this.engine = engine;
		this.basket = engine.entityNames.get("basket");
	}
	,updateNode: function(node,dt) {
		var t = node.fall.time += dt;
		var y = node.fall.startY - 981 * t * t * 0.5;
		var p = node.entity.get_position();
		var node1 = node.entity;
		var this1 = Module.Vector3(p.x,y,p.z);
		node1.setPosition(this1);
		if(y < -256) {
			systems_AudioSystem.instance.playSound("fail");
			this.engine.removeEntity(node.entity);
		}
		p = node.entity.get_position();
		var bp = this.basket.get_position();
		var bs = 32;
		if(p.y < bp.y + bs && p.y > bp.y - bs && p.x < bp.x + bs && p.x > bp.x - bs) {
			this.engine.removeEntity(node.entity);
			if(node.banana.good) {
				systems_AudioSystem.instance.playSound("pick");
				this.basket.get(components_Basket).score += 100;
				$(".score").text(Std.string(this.basket.get(components_Basket).score));
			} else {
				systems_AudioSystem.instance.playSound("hurt");
				this.basket.get(components_Basket).life -= 1;
				$(".life").text(Std.string(this.basket.get(components_Basket).life));
				if(this.basket.has(components_Colorizer)) {
					this.basket.get(components_Colorizer).time = 0;
				} else {
					var tmp = this.basket;
					var this2 = Module.Color(1,0,0,1);
					tmp.add(new components_Colorizer(0.5,this2));
				}
				if(this.basket.get(components_Basket).life <= 0) {
					Application.gameOver();
				}
			}
			var shaker;
			if(this.basket.has(components_Shaker)) {
				shaker = this.basket.get(components_Shaker);
				shaker.duration += 0.45;
			} else {
				shaker = new components_Shaker(0.45,false,10,0.05);
				this.basket.add(shaker);
			}
		}
	}
	,onNodeAdded: function(node) {
		node.fall.startY = node.entity.get_position().y;
		node.fall.time = 0;
		systems_AudioSystem.instance.playSound("fall");
	}
	,onNodeRemoved: function(node) {
	}
	,__class__: systems_FallSystem
});
var systems_GrowerNode = function() { };
systems_GrowerNode.__name__ = ["systems","GrowerNode"];
systems_GrowerNode._getComponents = function() {
	if(systems_GrowerNode._components == null) {
		systems_GrowerNode._components = new ash_ClassMap();
		var _this = systems_GrowerNode._components;
		var k = components_Grower;
		var name = Type.getClassName(k);
		var _this1 = _this.keyMap;
		if(__map_reserved[name] != null) {
			_this1.setReserved(name,k);
		...
		var _this2 = _this.valueMap;
		if(__map_reserved[name] != null) {
			_this2.setReserved(name,"grower");
		} else {
			_this2.h[name] = "grower";
		}
	}
	return systems_GrowerNode._components;
};
systems_GrowerNode.__super__ = ash_core_Node;
systems_GrowerNode.prototype = $extend(ash_core_Node.prototype,{
	__class__: systems_GrowerNode
});
var systems_GrowSystem = function() {
	this.duration = 2.5;
	ash_tools_ListIteratingSystem.call(this,systems_GrowerNode,$bind(this,this.updateNode),$bind(this,this.onNodeAdded),$bind(this,this.onNodeRemoved));
};
systems_GrowSystem.__name__ = ["systems","GrowSystem"];
systems_GrowSystem.__super__ = ash_tools_ListIteratingSystem;
systems_GrowSystem.prototype = $extend(ash_tools_ListIteratingSystem.prototype,{
	addToEngine: function(engine) {
		ash_tools_ListIteratingSystem.prototype.addToEngine.call(this,engine);
		this.engine = engine;
		this.tree = engine.entityNames.get("tree");
	}
	,updateNode: function(node,dt) {
		node.grower.time += dt;
		var f = node.grower.time / this.duration;
		var s = f * 0.35;
		var _this = node.entity;
		var this1 = Module.Vector3(s,s,s);
		var scale = this1;
		_this.node.setScale(scale);
		if(f > 1) {
			var e = node.entity;
			e.remove(components_Grower);
			e.add(new components_Shaker(1,true,10,0.1));
			var treeShaker;
			if(this.tree.has(components_Shaker)) {
				treeShaker = this.tree.get(components_Shaker);
				treeShaker.duration += 0.15;
			} else {
				treeShaker = new components_Shaker(0.15,false,0.9,0.01);
				this.tree.add(treeShaker);
			}
		}
	}
	,onNodeAdded: function(node) {
		node.grower.time = 0;
		var _this = node.entity;
		var this1 = Module.Vector3(0.01,0.01,0.01);
		var scale = this1;
		_this.node.setScale(scale);
	}
	,onNodeRemoved: function(node) {
	}
	,__class__: systems_GrowSystem
});
var systems_ShakerNode = function() { };
systems_ShakerNode.__name__ = ["systems","ShakerNode"];
systems_ShakerNode._getComponents = function() {
	if(systems_ShakerNode._components == null) {
		systems_ShakerNode._components = new ash_ClassMap();
		var _this = systems_ShakerNode._components;
		var k = components_Shaker;
		var name = Type.getClassName(k);
		var _this1 = _this.keyMap;
		if(__map_reserved[name] != null) {
			_this1.setReserved(name,k);
		...
		var _this2 = _this.valueMap;
		if(__map_reserved[name] != null) {
			_this2.setReserved(name,"shaker");
		} else {
			_this2.h[name] = "shaker";
		}
	}
	return systems_ShakerNode._components;
};
systems_ShakerNode.__super__ = ash_core_Node;
systems_ShakerNode.prototype = $extend(ash_core_Node.prototype,{
	__class__: systems_ShakerNode
});
var systems_ShakeSystem = function() {
	ash_tools_ListIteratingSystem.call(this,systems_ShakerNode,$bind(this,this.updateNode),$bind(this,this.onNodeAdded),$bind(this,this.onNodeRemoved));
};
systems_ShakeSystem.__name__ = ["systems","ShakeSystem"];
systems_ShakeSystem.__super__ = ash_tools_ListIteratingSystem;
systems_ShakeSystem.prototype = $extend(ash_tools_ListIteratingSystem.prototype,{
	addToEngine: function(engine) {
		ash_tools_ListIteratingSystem.prototype.addToEngine.call(this,engine);
		this.engine = engine;
	}
	,updateNode: function(node,dt) {
		...
		if(node.shaker.timeLeft < 0) {
			...
			var _this = node.entity;
			var angle = node.shaker.intensity * (Math.random() - 0.5);
			_this.node.setRotation2D(angle);
		}
		if(node.shaker.time > node.shaker.duration) {
			var e = node.entity;
			e.node.setRotation2D(0);
			e.remove(components_Shaker);
			if(node.shaker.fall) {
				e.add(new components_Fall());
			}
		}
	}
	,onNodeAdded: function(node) {
	...
	,__class__: systems_ShakeSystem
});
var systems_SpawnerNode = function() { };
systems_SpawnerNode.__name__ = ["systems","SpawnerNode"];
systems_SpawnerNode._getComponents = function() {
	if(systems_SpawnerNode._components == null) {
		systems_SpawnerNode._components = new ash_ClassMap();
		var _this = systems_SpawnerNode._components;
		var k = components_Spawner;
		var name = Type.getClassName(k);
		var _this1 = _this.keyMap;
		if(__map_reserved[name] != null) {
			_this1.setReserved(name,k);
		...
		var _this2 = _this.valueMap;
		if(__map_reserved[name] != null) {
			_this2.setReserved(name,"spawner");
		} else {
			_this2.h[name] = "spawner";
		}
	}
	return systems_SpawnerNode._components;
};
systems_SpawnerNode.__super__ = ash_core_Node;
systems_SpawnerNode.prototype = $extend(ash_core_Node.prototype,{
	__class__: systems_SpawnerNode
});
var systems_SpawnSystem = function() {
	this.timeLeft = 0;
	this.time = 0;
	ash_tools_ListIteratingSystem.call(this,systems_SpawnerNode,$bind(this,this.updateNode),$bind(this,this.onNodeAdded),$bind(this,this.onNodeRemoved));
};
systems_SpawnSystem.__name__ = ["systems","SpawnSystem"];
systems_SpawnSystem.__super__ = ash_tools_ListIteratingSystem;
systems_SpawnSystem.prototype = $extend(ash_tools_ListIteratingSystem.prototype,{
	addToEngine: function(engine) {
		ash_tools_ListIteratingSystem.prototype.addToEngine.call(this,engine);
		this.engine = engine;
	}
	,updateNode: function(node,dt) {
		...
		if(node.spawner.timeLeft <= 0) {
			...
			node.spawner.timeLeft = tmp + 3.5;
			this.spawn(node.entity.get_position());
		}
	}
	,onNodeAdded: function(node) {
		node.spawner.time = 0;
		var tmp = Math.random();
		node.spawner.timeLeft = tmp * 2;
	}
	,onNodeRemoved: function(node) {
	}
	,spawn: function(position) {
		var e = new gengine_Entity();
		e.add(new components_Banana());
		e.add(new gengine_components_StaticSprite2D());
		e.add(new components_Grower());
		var ss = e.get(gengine_components_StaticSprite2D);
		var sprite = gengine.getResourceCache().getSprite2D("banana.png",true);
		ss.object.setSprite(sprite);
		ss.object.setLayer(2);
		ss.object.setUseHotSpot(true);
		var this1 = Module.Vector2(0.5,1);
		ss.object.setHotSpot(this1);
		var this2 = Module.Vector3(0.25,0.25,0.25);
		var scale = this2;
		e.node.setScale(scale);
		e.set_position(position);
		this.engine.addEntity(e);
		if(Std.random(10) < 2) {
			e.get(components_Banana).good = false;
			var tmp = e.get(gengine_components_StaticSprite2D);
			var this3 = Module.Color(0.9,0.1,0.2,1);
			tmp.setColor(this3);
		}
	}
	,__class__: systems_SpawnSystem
});
var $_, $fid = 0;
